...
export const Steps = ({ steps, activeIdx, showProgress }: { steps: string[], activeIdx: number, showProgress: boolean }) => (
  <StepsContainer>
    {
      steps.map((stepText, idx) => {
        return <Step key={idx} idx={idx} activeIdx={activeIdx} isLast={idx == steps.length - 1} showProgress={showProgress}>{stepText}</Step>
      })
    }
  </StepsContainer>
)

const Step = ({ children, idx, activeIdx, isLast, showProgress }: { children: ReactNode, idx: number, activeIdx: number, isLast: boolean, showProgress: boolean }) => {
