...
${(props) =>
    props.large ? 'font-size: 125%;' : props.small ? 'font-size: 85%;' : ''}
  ...
  @media ${({ theme }) => theme.screenMax.mobileL} {
    ...
    padding: ${({ border, small }) => {
      ...
      return `${(topBottomPaddingBase - padded) / reducerBase}px ${
        (leftRightPaddingBase - padded) / reducerBase
      }px !important`
    }};
...
const DefaultButton = styled(BaseButton)`
  ...
  ${({ border, borderColor, theme }) =>
    border
      ? 'border: 2px solid ' +
        (borderColor ? borderColor : theme.lightBorder) +
        ';'
      : ''}
...
const ButtonContent = styled.div<{
  ...
  justify-content: ${({ justify }) =>
    justify ? justify : JustifyContent.Center};
  align-items: center;
  ${(props) =>
