...
interface IPreregisteredRedirectModalProps {
  isOpen: boolean
  processStartDate: string
  onClose: () => void
}

export const PreregisteredRedirectModal = ({
  isOpen,
  processStartDate,
  onClose,
}: IPreregisteredRedirectModalProps) => {
      ...
      <FlexContainer direction={FlexDirection.Column} alignItem={FlexAlignItem.Center}>
        ...
        <TextContainer>
          <Typography variant={TypographyVariant.H1} align={TextAlign.Center}>
            {i18n.t("vote.auth.preregister_redirect_dialog.title")}
          </Typography>
        ...
        <Button positive onClick={onClose}>
          {i18n.t('vote.auth.close_preregister_redirect_modal')}
        </Button>
      </FlexContainer>
    </Modal>
  )
