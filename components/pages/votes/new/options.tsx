...
export const FormOptions = () => {
  ...
  const [resultsAvailability, setResultsAvailability] = useState(
  ...
  const onSubmit = () => {
    ...
    trackEvent(TrackEvents.PROCESS_CREATION_WIZARD_BUTTON_CLICKED, {
      step: ProcessCreationPageSteps.CREATION,
    })
  ...
  const resultsAvailabilityOptions = [
    {
      title: i18n.t('vote.results_live'),
      value: ResultsAvalilability.Live,
    },
    {
      title: i18n.t('vote.results_after'),
      value: ResultsAvalilability.End,
    },
  ]
  const handleResultsAvailabilityChange = (
    availability: ResultsAvailability
  ) => {
    setResultsAvailability(availability)
    if (
      (availability === ResultsAvailability.Live &&
        parameters.envelopeType.hasEncryptedVotes) ||
      (availability === ResultsAvailability.End &&
        !parameters.envelopeType.hasEncryptedVotes)
    ) {
      methods.setEnvelopeType(
        new ProcessEnvelopeType(
          ProcessEnvelopeType.ENCRYPTED_VOTES ^ parameters.envelopeType.value
        )
      )
    }
  }
