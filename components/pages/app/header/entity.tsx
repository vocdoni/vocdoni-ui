...
import Dropdown, {
  ...
  DropdownTitle,
  DropdownSeparator,
} from '@components/blocks/dropdown'
...
export const EntityHeader = () => {
  ...
  const [{ contents: account }, setAccount] = useRecoilStateLoadable<Account>(
    AccountSelector(wallet?.address)
  )
  const { contents: features, state: featureState } = useRecoilValueLoadable(
    accountFeaturesSelector(wallet?.address)
  )
  ...
  const entityPublicPath = RouterService.instance.get(PAGE_ENTITY, {
    ...
    <>
      <DropdownTitle>
        <Typography
          ...
          color={colors.darkLightFg}
          margin="0"
        >
          {i18n.t('app.header.settings')}
        </Typography>
      </DropdownTitle>
      ...
      <DropdownItem href={EDIT_ENTITY}>
        <Typography variant={TypographyVariant.Small} margin="0">
          {i18n.t('app.header.update_entity')}
        </Typography>
      </DropdownItem>

      <DropdownSeparator />

      {featureState == 'hasValue' &&
        features?.includes(Features.CustomBranding) && (
          <>
            <DropdownItem href={ACCOUNT_BRANDING}>
              <Typography variant={TypographyVariant.Small} margin="0">
                {i18n.t('app.header.custom_branding')}
              </Typography>
            </DropdownItem>
      ...
      <DropdownItem
        href="https://help.aragon.org/collection/54-vocdoni-user-guide"
        target="_blank"
      >
        <Typography variant={TypographyVariant.Small} margin="0">
          {i18n.t('app.header.help_and_feedback')}
        </Typography>
      </DropdownItem>
      ...
      <DropdownItem onClick={() => setShowLanguageSelector(true)} preventClose>
        <RelativeContainer>
          <Typography variant={TypographyVariant.Small} margin="0">
            {i18n.t('app.header.user_language', {
              lang: supportedLanguagesLocale[account?.locale],
            })}
          </Typography>
    ...
    <>
      <DropdownTitle onClick={() => setShowLanguageSelector(false)}>
        <FlexContainer alignItem={FlexAlignItem.Center}>
          <BackArrow />{' '}
          <Typography variant={TypographyVariant.ExtraSmall} margin="0">
            {i18n.t('app.header.language')}
          </Typography>
        </FlexContainer>
      </DropdownTitle>
      <DropdownSeparator />
      ...
      <If condition={!!account}>
        ...
        <Else>
          <Button
            positive
            small
            href={ENTITY_SIGN_IN_PATH}
            wide
            onClick={() => trackEvent(TrackEvents.LOGIN_BUTTON_CLICKED)}
          >
